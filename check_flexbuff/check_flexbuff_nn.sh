...
if [ $# -eq 0 ]; then
  ...
  IFS=$'\n' sorted_extant_correlated_sessions=($(sort <<<"${extant_correlated_sessions[*]}"))
  unset IFS
  ...
  if [[ "${#extant_correlated_sessions[@]}" -gt 0 ]]; then
    ...
    for sess in "${sorted_extant_correlated_sessions[@]}"; do
      echo ${sess}
    done
    ...
    # Delete matches...
    ###################
    read -p "Would you like to delete them? [y/n] " -n 1 -r
    if [[ ${REPLY} =~ ^[Yy]$ ]]; then
